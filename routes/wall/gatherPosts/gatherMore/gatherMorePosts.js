...
mongo.connect(config.get("mongoURI"),  { useNewUrlParser: true }, { useUnifiedTopology: true }, cors(), (err, db) => {
    router.get("/", (req, resp) => {
        ...
        collection.distinct("wallPosts", { $and: [ { "wallPosts.id": { $nin: alreadyPooled.filter(doc => {
            return doc;
        })}}, { "wallPosts.newData.id": { $nin: alreadyPooled.filter(doc => {
            return doc;
        })}}]}, (err, posts) => {
            if (err) {
                console.log(err);
                ...
                for (let index = 0; index < posts.slice(0,4).length; index++) {
